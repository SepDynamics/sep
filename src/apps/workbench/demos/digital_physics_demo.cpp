    ...
    void DigitalPhysicsDemo::on_load(sep::core::Engine* engine, sep::SimpleRenderer* renderer)
    {
        ...
        for (std::size_t y = 0; y < height_; ++y)
        {
            for (std::size_t x = 0; x < width_; ++x)
            {
                ...
                cell.attributes[0] = static_cast<float>(std::rand() % 2);
            }
        }
    }

void DigitalPhysicsDemo::on_update(float) {
    std::vector<sep::compat::PatternData> next = grid_;
    ...
    auto alive = [&](int x, int y) -> bool {
        ...
        return grid_[index(static_cast<std::size_t>(x), static_cast<std::size_t>(y))]
                   .attributes[0] > 0.5f;
    };
    ...
    for (std::size_t y = 0; y < height_; ++y) {
        for (std::size_t x = 0; x < width_; ++x) {
            ...
            bool current = grid_[index(x, y)].attributes[0] > 0.5f;
            bool next_state;
            if (current) {
            ...
            next[index(x, y)].attributes[0] = next_state ? 1.0f : 0.0f;
        }
    }
