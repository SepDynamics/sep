...
class Connection
 ...
 public:
  Connection(asio::io_context& io_context, Handler* handler, const std::string& server_name,
             std::tuple<Middlewares...>* middlewares,
             std::function<std::string()>& get_cached_date_str_f, detail::task_timer& task_timer,
             typename Adaptor::context* adaptor_ctx_, std::atomic<unsigned int>& queue_length)
  ...
  void handle_url() {
    ...
    // if no route is found for the request method, return the response without parsing or
  ...
  void handle() {
    // TODO(EDev): cancel_deadline_timer should be looked into. Integration with
    // handle_url() is deferred until the connection lifecycle is revisited.
    ...
    if (req_.check_version(1, 1))  // HTTP/1.1
    {
      if (!req_.headers.count("host")) {
        ...
        if (req_.get_header_value("upgrade").find("h2") == 0) {
          // TODO(ipkn): HTTP/2 support is planned but not yet implemented.
          // currently, ignore upgrade header
        } else {
          detail::middleware_call_helper<detail::middleware_call_criteria_only_global, 0,
                                         decltype(ctx_), decltype(*middlewares_)>({}, *middlewares_,
                                                                                  req_, res, ctx_);
    ...
    CROW_LOG_INFO << "Request: " << utility::lexical_cast<std::string>(adaptor_.remote_endpoint())
                  << " " << this << " HTTP/" << (char)(req_.http_ver_major + '0') << "."
                  << (char)(req_.http_ver_minor + '0') << ' ' << method_name(req_.method) << " "
                  << req_.url;
 ...
 private:
  void prepare_buffers() {
    ...
    // TODO(EDev): HTTP version in status codes should be dynamic. For now this
    // map is kept in sync with common.h/status.
    ...
    if (res.code >= 400 && res.body.empty()) res.body = statusCodes[res.code].substr(9);
  ...
  void do_write_static() {
    ...
    if (close_connection_) {
      auto sec = adaptor_.shutdown_readwrite();
      if (sec)
        CROW_LOG_ERROR << "Shutdown failed: " << sec.message();
      auto cec = adaptor_.close();
      if (cec)
        CROW_LOG_ERROR << "Close failed: " << cec.message();
  ...
  void do_read() {
    auto self = this->shared_from_this();
    adaptor_.socket().async_read_some(asio::buffer(buffer_), [self](const error_code& ec,
      ...
      if (error_while_reading) {
        ...
        self->parser_.done();
        {
          auto sec = self->adaptor_.shutdown_read();
          if (sec)
            CROW_LOG_ERROR << "Shutdown failed: " << sec.message();
          auto cec = self->adaptor_.close();
          if (cec)
            CROW_LOG_ERROR << "Close failed: " << cec.message();
        }
        CROW_LOG_DEBUG << self << " from read(1) with description: \""
                       << http_errno_description(static_cast<http_errno>(self->parser_.http_errno))
                       << '\"';
                      ...
                      [self](const error_code& ec, std::size_t /*bytes_transferred*/) {
                        ...
                        if (!ec) {
                          if (self->close_connection_) {
                            auto sec = self->adaptor_.shutdown_write();
                            if (sec)
                              CROW_LOG_ERROR << "Shutdown failed: " << sec.message();
                            auto cec = self->adaptor_.close();
                            if (cec)
  ...
  void start_deadline(/*int timeout = 5*/) {
    ...
    task_id_ = task_timer_.schedule([self] {
      ...
      auto sec = self->adaptor_.shutdown_readwrite();
      if (sec)
        CROW_LOG_ERROR << "Shutdown failed: " << sec.message();
      auto cec = self->adaptor_.close();
      if (cec)
