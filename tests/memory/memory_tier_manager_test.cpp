...
// Tests operate on fairly small tiers where integer rounding can leave a
// minimal non-zero utilization after promotions or defragmentation.  The
// production epsilon in memory_tier.hpp (1e-5f) is intentionally small so that
// tiny allocations remain visible.  To avoid spurious failures we use a more
// permissive tolerance for unit tests.
constexpr float kTestUtilizationTolerance = 1e-3f;
...
TEST(MemoryTierManagerTest, PromotionAndDemotion) {
    ...
    EXPECT_NEAR(ltm_util, 0.0f, kTestUtilizationTolerance)
    ...
    EXPECT_NEAR(mtm_util, 0.0f, kTestUtilizationTolerance)
...
TEST(MemoryTierManagerTest, OptimizeBlocksPromotionDemotion) {
    ...
    float mtm_util = mgr.getTierUtilization(MemoryTierEnum::MTM);
    EXPECT_NEAR(stm_util, 0.0f, kTestUtilizationTolerance)
...
TEST(MemoryTierManagerTest, UtilizationResetsAfterPromotionDemotion) {
    ...
    EXPECT_LT(mgr.getTierUtilization(MemoryTierEnum::MTM), kTestUtilizationTolerance);
    EXPECT_LT(mgr.getTierUtilization(MemoryTierEnum::LTM), kTestUtilizationTolerance);
}
