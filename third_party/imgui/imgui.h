...
struct ImVector
{
    ...
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = (T*)IM_ALLOC((size_t)new_capacity * sizeof(T)); if (Data) { for (int i = 0; i < Size; i++) new_data[i] = Data[i]; IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }
    ...
    inline T*           erase(const T* it)                  { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; for (ptrdiff_t i = off; i < Size - 1; i++) Data[i] = Data[i + 1]; Size--; return Data + off; }
    inline T*          erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last >= it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; if (off + count < Size) for (int i = off; i < Size - count; i++) Data[i] = Data[i + count]; Size -= (int)count; return Data + off; }
    inline T*           erase_unsorted(const T* it)         { IM_ASSERT(it >= Data && it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) Data[off] = Data[Size - 1]; Size--; return Data + off; }
    inline T*           insert(const T* it, const T& v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) for (ptrdiff_t i = Size; i > off; i--) Data[i] = Data[i - 1]; Data[off] = v; Size++; return Data + off; }
...
struct ImGuiMultiSelectIO
{
    ...
    ImGuiMultiSelectIO() : Requests(), RangeSrcItem(-1), NavIdItem(-1), NavIdSelected(false), RangeSrcReset(false), ItemsCount(0) {}
...
struct ImDrawList
{
    ...
    {
        if (_Path.Size == 0 || (_Path.Data[_Path.Size - 1].x != pos.x || _Path.Data[_Path.Size - 1].y != pos.y))
            _Path.push_back(pos);
    }
