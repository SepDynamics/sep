...
// QFH analysis result
struct QFHResult {
    shim::vector<QFHEvent> events;
    shim::vector<QFHAggregateEvent> aggregated_events;
...
public:
    ...
    // Analyze bit pattern
    QFHResult analyze(const shim::vector<uint8_t>& bits);
    ...
    static shim::vector<uint8_t> convertToBits(const shim::vector<uint32_t>& values);
