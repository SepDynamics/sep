...
int main()
{
    ...
    // Make sure we have a valid context before initializing GLEW
    if (glGetString(GL_VERSION) == NULL)
    {
        std::cerr << "CRITICAL ERROR: No OpenGL context found before glewInit()!" << std::endl;
        glfwDestroyWindow(window);
    ...
    // Explicitly clear any OpenGL errors before GLEW init
    ...
    // Clear any error generated by GLEW initialization
    // (this is normal as GLEW sometimes generates GL_INVALID_ENUM)
    while (glGetError() != GL_NO_ERROR)
    {
    }
    ...
    if (err != GLEW_OK)
    {
        ...
        // Set imgui_available to false since GLEW failed
        imgui_available = false;
    }
    else
    {
        imgui_available = true;  // Set to true only if GLEW initialized successfully
