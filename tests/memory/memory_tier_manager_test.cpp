...
// Tests rely on a slightly looser tolerance than the core engine uses so that
// tiny rounding differences after promotions or defragmentation do not cause
// spurious failures.  A value of 1e-3 comfortably covers residual utilization
// values such as 0.000244 that appear with 4 KiB tiers while still catching
// meaningful regressions.
constexpr float kTestUtilizationEpsilon = 1e-3f;
...
TEST(MemoryTierManagerTest, PromotionAndDemotion) {
    ...
    EXPECT_NEAR(ltm_util, 0.0f, kTestUtilizationEpsilon)
    ...
    EXPECT_NEAR(mtm_util, 0.0f, kTestUtilizationEpsilon)
...
TEST(MemoryTierManagerTest, OptimizeBlocksPromotionDemotion) {
    ...
    float mtm_util = mgr.getTierUtilization(MemoryTierEnum::MTM);
    EXPECT_NEAR(stm_util, 0.0f, kTestUtilizationEpsilon)
...
TEST(MemoryTierManagerTest, UtilizationResetsAfterPromotionDemotion) {
    ...
    EXPECT_LT(mgr.getTierUtilization(MemoryTierEnum::MTM), kTestUtilizationEpsilon);
    EXPECT_LT(mgr.getTierUtilization(MemoryTierEnum::LTM), kTestUtilizationEpsilon);
}
