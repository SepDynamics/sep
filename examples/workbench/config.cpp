...
bool Config::load(const std::filesystem::path& path) {
    try {
        ...
        auto& renderer = json["renderer"]["cycles"];
        renderer_ = { { renderer["samples"].get<int>(),
                        renderer["denoising"].get<bool>(),
                        renderer["device"].get<std::string>() } };
...
bool Config::save(const std::filesystem::path& path) const {
    try {
        ...
        json["demos"]["genesis_pattern"] = {
            {"initial_pattern",
             { {"dimensions", { genesis_pattern_.initial_pattern.dimensions[0],
        ...
        json["renderer"]["cycles"] = { {"samples", renderer_.cycles.samples},
