...
// Include base CUDA types and standard headers
#include "cuda_base.h"
...
// Function declarations with implementations for both CUDA and non-CUDA environments
    ...
    inline cudaError_t cudaMemcpyAsync(void* dst, const void* src, size_t size,
    ...
    inline cudaError_t cudaMallocManaged(void**, size_t, unsigned int flags = SEP_cudaMemAttachGlobal) { 
...
inline cudaError_t SEP_cudaMemcpyAsync(void* dst, const void* src, size_t size, cudaMemcpyKind kind, cudaStream_t stream) { 
    return cudaMemcpyAsync(dst, src, size, kind, stream); 
}
...
inline cudaError_t SEP_cudaMallocManaged(void** ptr, size_t size, unsigned int flags = SEP_cudaMemAttachGlobal) { 
    return cudaMallocManaged(ptr, size, flags); 
}
inline cudaError_t SEP_cudaHostAlloc(void** ptr, size_t size, unsigned int flags) { 
    return cudaHostAlloc(ptr, size, flags); 
}
inline cudaError_t SEP_cudaStreamAttachMemAsync(cudaStream_t stream, void* ptr, size_t size, unsigned int flags) { 
    return cudaStreamAttachMemAsync(stream, ptr, size, flags); 
}
