...
2. **Consolidate application code** – Move `apps/common` into `apps/oanda_trader` or a shared `ui` module if additional GUIs appear. *(Partially completed: helpers now live in `src/ui`.)*
