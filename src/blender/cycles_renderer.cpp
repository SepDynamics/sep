...
SEPResult CyclesRenderer::renderScene(const RenderParams& params) {
    ...
    try {
        ...
        if (cycles_scene_->integrator) {
            cycles_scene_->integrator->set_aa_samples(params.samples);
            cycles_scene_->integrator->set_use_denoise(params.use_denoising);
            cycles_scene_->integrator->set_denoiser_prefilter(::ccl::DENOISER_PREFILTER_FAST);
            cycles_scene_->integrator->set_denoise_start_sample(0);
            cycles_scene_->integrator->set_denoiser_type(::ccl::DENOISER_OPENIMAGEDENOISE);
        }
