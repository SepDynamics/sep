...
SEPResult CyclesRenderer::renderScene(const RenderParams& params) {
    ...
    try {
        ...
        if (cam) {
            ...
            ::ccl::array<::ccl::Transform> empty_motion;
            cam->set_motion(empty_motion);
            cam->set_shuttertime(0.0f);
            cam->set_rolling_shutter_type(::ccl::ROLLING_SHUTTER_NONE);
            cam->set_camera_type(::ccl::CAMERA_PERSPECTIVE);
            cam->set_use_spherical_stereo(false);
            cam->set_stereo_eye(::ccl::STEREO_NONE);
        ...
        // Configure render settings (only supported parameters)
