...
namespace sep::api {
                                                    ...
                                                    const shim::string& layer)
    {
        ...
        do
        {
            ret_code = static_cast<int>(sep_process_context(context_json.c_str(), layer.c_str(),
                                                            result_buffer.data(), buffer_size));

            if (ret_code == static_cast<int>(ErrorCode::BufferTooSmall))
            {  // Buffer too small
