...
class IConnectionManager {
 public:
  ...
  virtual std::shared_ptr<ISocket> getConnection(const shim::string &host, uint16_t port) = 0;
  virtual Status releaseConnection(std::shared_ptr<ISocket> socket) = 0;
  virtual Status closeAll() = 0;
  virtual shim::string getStats() const = 0;

  protected:
     virtual std::shared_ptr<ISocket> createConnection(const shim::string &host, uint16_t port) = 0;
