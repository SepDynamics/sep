                                                              ...
                                                              std::vector<sep::pattern::PatternData>& output)
{
    ...
    for (std::size_t i = 0; i < input.size(); ++i)
    {
        ...
        output[i].quantum_state.memory_tier =
            tier_helper.determineMemoryTier(output[i].quantum_state.coherence,
                                            output[i].quantum_state.stability,
                                            output[i].generation);
                                                                        ...
                                                                        const sep::pattern::PatternData& pattern2)
{
    ...
    float coherence_diff =
        std::abs(pattern1.quantum_state.coherence - pattern2.quantum_state.coherence);
    float stability_diff =
        std::abs(pattern1.quantum_state.stability - pattern2.quantum_state.stability);
    float entropy_diff =
        std::abs(pattern1.quantum_state.entropy - pattern2.quantum_state.entropy);
