...
int main() {
    ...
    // 2. Initialize the Renderer (The Window)
    ...
    while (!renderer.shouldClose()) {
        genesis_demo->update(0.016f); // Update with a fixed delta time
        genesis_demo->render();       // Render the demo's state

        renderer.present(); // Swap buffers, handle events
