...
namespace api {
    ...
    inline nlohmann::json parse_json(const sep::string& str)
    {
        std::string std_str(str.c_str(), str.size());
        return nlohmann::json::parse(std_str);
    }

inline nlohmann::json parse_json(const std::string& str) { return nlohmann::json::parse(str); }
...
inline std::string to_std_string(const sep::string& str)
{
