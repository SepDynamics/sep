 ...
 public:
  ...
  Impl() : running_(false), latency_window_size_(1000) {
    // Create events for timing
        CUDA_CHECK(sep::cuda::SEP_cudaEventCreate(&start_event_));
    CUDA_CHECK(sep::cuda::SEP_cudaEventCreate(&stop_event_));
  ...
  ~Impl() {
    stopCollection();
    if (start_event_) {
      CUDA_CHECK(sep::cuda::SEP_cudaEventDestroy(start_event_));
    }
    if (stop_event_) {
      CUDA_CHECK(sep::cuda::SEP_cudaEventDestroy(stop_event_));
    }
  }
  ...
  void recordKernelStart() {
    if (start_event_) {
      CUDA_CHECK(sep::cuda::SEP_cudaEventRecord(start_event_, nullptr));
    }
  }

  void recordKernelStop() {
    if (stop_event_) {
            CUDA_CHECK(sep::cuda::SEP_cudaEventRecord(stop_event_, nullptr));
      CUDA_CHECK(sep::cuda::SEP_cudaEventSynchronize(stop_event_));

      float elapsed_time = 0.0f;
      if (start_event_ && stop_event_) {
        CUDA_CHECK(sep::cuda::SEP_cudaEventElapsedTime(&elapsed_time, start_event_, stop_event_));
      }
  ...
  void collectMetrics() {
    ...
    CUDA_CHECK(sep::cuda::SEP_cudaMemGetInfo(&free_mem, &total_mem));
