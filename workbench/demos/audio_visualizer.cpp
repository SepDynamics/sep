        ...
        void AudioVisualizerDemo::handleKeyboard(unsigned char key)
        {
            ...
            switch (key)
            {
                ...
                case 'F':  // Decrease frequency scale
                    pattern_mapping_.frequency_scale =
                        std::max(0.1f, pattern_mapping_.frequency_scale - SCALE_STEP);
                    break;
                case 'a':  // Increase amplitude scale
                    pattern_mapping_.amplitude_scale += SCALE_STEP;
                    break;
                case 'A':  // Decrease amplitude scale
                    pattern_mapping_.amplitude_scale =
                        std::max(0.1f, pattern_mapping_.amplitude_scale - SCALE_STEP);
                    break;
                case 'e':  // Increase evolution sensitivity
                    pattern_mapping_.evolution_sensitivity += SCALE_STEP;
                    pattern_mapping_.evolution_sensitivity =
                        std::min(1.0f, pattern_mapping_.evolution_sensitivity);
                    break;
                case 'E':  // Decrease evolution sensitivity
                    pattern_mapping_.evolution_sensitivity =
                        std::max(0.1f, pattern_mapping_.evolution_sensitivity - SCALE_STEP);
                    break;
