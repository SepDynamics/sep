...
struct Stream::Impl {
  ...
  void synchronize() {
      if (stream_handle_) sep::cuda::SEP_cudaStreamSynchronize(stream_handle_);
  }
  void wait(void* event) {
    if (stream_handle_ && event)
        sep::cuda::SEP_cudaStreamWaitEvent(stream_handle_, static_cast<cudaEvent_t>(event), 0);
  }
  void record(void* event) {
    if (stream_handle_ && event)
        sep::cuda::SEP_cudaEventRecord(static_cast<cudaEvent_t>(event), stream_handle_);
  }
...
std::shared_ptr<Stream> Stream::create(sep::StreamFlags flags) {
  ...
  cudaError_t err = sep::cuda::SEP_cudaStreamCreateWithFlags(&cuda_stream, cuda_flags);
  if (err != cudaSuccess) {
