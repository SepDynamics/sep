...
int main()
{
    ...
    glfwSetErrorCallback([](int error, const char* description) {
        static std::ofstream log_file("glfw.log", std::ios::app);
        log_file << "GLFW Error " << error << ": " << description << std::endl;
    });
    ...
    // Create a global flag to track if ImGui is available (moved up from below)
    bool imgui_available = false;

    // 2. Initialize GLAD

    // CRITICAL: Check for an OpenGL context *before* GLAD
    if (glGetString(GL_VERSION) == NULL)
    {
        std::cerr << "CRITICAL ERROR: No OpenGL context found before gladLoadGLLoader()!"
                  << std::endl;
    ...
    // Set up a simple rendering state - do this before GLAD
    glClearColor(0.2f, 0.3f, 0.3f, 1.0f);

    // Make sure we have a valid context before initializing GLAD
    if (glGetString(GL_VERSION) == NULL)
    {
        std::cerr << "CRITICAL ERROR: No OpenGL context found before gladLoadGLLoader()!"
                  << std::endl;
    ...
    // Explicitly clear any OpenGL errors before GLAD init
    ...
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))
    {
        ...
        // Set imgui_available to false since GLAD failed
        imgui_available = false;
    }
    else
    {
        imgui_available = true;  // Set to true only if GLAD initialized successfully
        std::cout << "GLAD Initialized Successfully." << std::endl;
    }

    // Check for required OpenGL 3.3 support
    if (GLVersion.major < 3 || (GLVersion.major == 3 && GLVersion.minor < 3))
    {
        std::cerr << "Required OpenGL 3.3 support missing" << std::endl;
    }

    // Clear any errors that might have been generated by GLAD init
    ...
    // Skip ImGui if GLAD had errors - this prevents the segfault
    if (!imgui_available)
    {
        std::cerr << "GLAD initialization failed, skipping ImGui..." << std::endl;
        logger->warn("GLAD initialization failed, operating in minimal mode");
    }
    else
    {
