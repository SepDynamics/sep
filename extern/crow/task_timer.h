...
class task_timer {
 ...
 public:
  task_timer(asio::io_context& io_context,
      ...
      : io_context_(io_context), timer_(io_context_), tick_length_ms_(tick_length) {
  ...
  identifier_type schedule(const task_type& task) { return schedule(task, get_default_timeout()); }
  ...
  identifier_type schedule(const task_type& task, uint8_t timeout) {
    tasks_.insert({++highest_id_, {clock_type::now() + (timeout * tick_length_ms_), task}});
