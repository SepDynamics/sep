...
bool Config::load(const std::filesystem::path &path) {
  try {
    ...
    window_ = {window["title"].get<std::string>(), window["width"].get<int>(),
               window["height"].get<int>(), window["fullscreen"].get<bool>(),
               window["vsync"].get<bool>()};

    // Engine config
    auto &engine = json["engine"];
    engine_ = {engine["cuda_enabled"].get<bool>(),
    ...
    genesis_pattern_ = {{{initial["dimensions"][0].get<int>(),
    ...
    memory_garden_ = {{layout["stm_radius"].get<float>(),
    ...
    annealing_ = {anneal["particle_count"].get<int>(),
    ...
    json["demos"]["audio_visualizer"] = {
        ...
        {"pattern_mapping",
         {{"frequency_scale",
          ...
          {"amplitude_scale",
           audio_visualizer_.pattern_mapping.amplitude_scale},
