...
bool operator==(const std::__cxx11::basic_string<char>& lhs, const sep::shim::string& rhs) {
...
public:
    ...
    float calculateFitness(const Pattern& pattern) const {
        ...
        float fitness = (coherence_fitness + stability_fitness + entropy_penalty + diversity_bonus) * pressure_factor;
...
private:
    ...
    float calculatePatternDistance(const Pattern& p1, const Pattern& p2) const {
        ...
        float state_distance = std::abs(p1.quantum_state.coherence - p2.quantum_state.coherence) +
    ...
    void updateStats(const std::vector<Pattern>& patterns) {
        ...
        if (!patterns.empty()) {
            ...
            stats.average_fitness = std::accumulate(fitness_values.begin(), fitness_values.end(), 0.0f) / patterns.size();
            stats.best_fitness = *std::max_element(fitness_values.begin(), fitness_values.end());
            stats.worst_fitness = *std::min_element(fitness_values.begin(), fitness_values.end());
    ...
    std::string generatePatternId() const {
        ...
        return "evo_pat_" + std::to_string(getCurrentTimestamp()) + "_" + std::to_string(counter.fetch_add(1));
    }
