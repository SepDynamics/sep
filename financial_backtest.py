...
def main():
    ...
    parser.add_argument("market_data_file", help="Path to the market_data.csv file.")
    ...
    # Load market data
    market_df = pd.read_csv(args.market_data_file, parse_dates=['Date'])
    market_df.rename(columns={'Date': 'date', 'Close': 'close'}, inplace=True)

    # --- Data Preparation for Multi-Resolution ---
    # Assuming data until end of 2024 is daily, and 2025 is by the minute
    df_daily = market_df[market_df['date'].dt.year < 2025]
    df_minute = market_df[market_df['date'].dt.year >= 2025]

    # Merge daily metrics with daily market data
    df_daily_merged = pd.merge(metrics_df, df_daily, on='date')

    # Calculate the predictive gauge for daily data
    df_daily_merged['gauge'] = calculate_gauge(df_daily_merged)

    # --- Run Analyses ---
    run_analysis_a(df_daily_merged.copy())
    run_analysis_b(df_daily_merged.copy())
    run_analysis_c(df_daily_merged.copy(), df_minute.copy())
