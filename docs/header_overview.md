...
- **`core/core_types.h`** defines fundamental data structures such as `PatternData`. It now initializes members in declaration order to avoid `-Wreorder` warnings. This header includes `cuda/math_common.h` for shared math utilities.
...
- **`cuda/math_common.h`** pulls in `pattern/pattern_common.h` and exposes device-friendly math helpers. It also mirrors the threshold constants for CUDA kernels.
